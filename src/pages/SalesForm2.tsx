...
const SalesForm: React.FC<SalesFormProps> = ({ onBack }) => {
        ...
        const fetchProducts = async () => {
            try {
                ...
                const [response1, response2] = await Promise.all([
                    axios.get(`${baseUrl}${apiConfig.endpoints.productsMama}/1`),
                    axios.get(`${baseUrl}${apiConfig.endpoints.productsMama}/2`)
                ]);
    ...
    const handleSaveSale = async () => {
        ...
        try {
            ...
            // Obtener baseUrl
            const baseUrl = await apiConfig.getApiUrl(apiConfig.endpoints.form2);

            // Enviar solicitud al endpoint de ventas
            const response = await axios.post(`${baseUrl}${apiConfig.endpoints.form2}`, saleData);
